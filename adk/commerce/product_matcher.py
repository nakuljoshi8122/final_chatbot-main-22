...
GENERIC_NAME_TOKENS = frozenset({
    "boost", "prime", "shopassist", "running", "training", "classic", "original",
...
def should_use_browse_flow(query: str, previous_browse: str = "") -> bool:
    try:
        from commerce.prior_items import is_describe_prior_items_query
    except ImportError:
        from commerce.prior_items import is_describe_prior_items_query
    if is_best_recommendation_query(query):
        return False
...
def _description(product: dict) -> str:
    ...
    return f"{sport} gear" if sport else "ShopAssist gear"
...
def format_product_card(product: dict) -> dict:
    ...
    return {
        ...
        "url": f"https://shopassist.local/{re.sub(r'[^a-z0-9]+', '-', product['name'].lower()).strip('-')}",
        "sport": product.get("sport", product.get("type", "")),
