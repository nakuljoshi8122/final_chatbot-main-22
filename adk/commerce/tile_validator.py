"""Validate agent TILES against the real ShopAssist catalogue + pagination."""
...
def resolve_browse_query(
    ...
    """
    try:
        from commerce.conversation_context import resolve_browse_query_enhanced
        from commerce.browse_filters import on_browse_turn
    except ImportError:
        from commerce.conversation_context import resolve_browse_query_enhanced
        from commerce.browse_filters import on_browse_turn
...
def _update_browse_session(
    ...
    try:
        from commerce.conversation_context import record_browse_turn
    except ImportError:
        from commerce.conversation_context import record_browse_turn
...
def match_products_excluding(
    ...
    try:
        from commerce.browse_filters import get_session_filters, apply_filters_to_cards
    except ImportError:
        from commerce.browse_filters import get_session_filters, apply_filters_to_cards
...
def build_catalog_tiles(
    ...
    try:
        from commerce.browse_filters import get_session_filters, match_products_constrained
    except ImportError:
        from commerce.browse_filters import get_session_filters, match_products_constrained
...
def rebuild_response_with_tiles(
    ...
    *,
    ...
    try:
        from commerce.visual_handoff import ensure_visual_handoff
    except ImportError:
        from commerce.visual_handoff import ensure_visual_handoff
...
def sanitize_shopassist_response(
    response_text: str,
    user_query: str = "",
    session_id: str = "",
    conversation_history: Optional[dict] = None,
    ...
    try:
        from commerce.browse_filters import apply_filters_to_tiles, get_session_filters, build_no_match_line
    except ImportError:
        from commerce.browse_filters import apply_filters_to_tiles, get_session_filters, build_no_match_line
    ...
    if is_best_recommendation_query(user_query):
        try:
            from commerce.best_recommendation import build_best_recommendation_response
        except ImportError:
            from commerce.best_recommendation import build_best_recommendation_response
    ...
    try:
        from commerce.prior_items import (
