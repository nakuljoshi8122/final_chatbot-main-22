...
IMAGES_JSON = BOUTIQUE_PRODUCT_IMAGES_JSON
...
def parse_kb_products(kb_path: Path = KB_PATH) -> list[dict[str, Any]]:
    ...
    if kb_path.exists():
        ...
        for part in parts:
            ...
            try:
                try:
                    from stores.store_scope import chunk_matches_store, get_current_store
                except ImportError:
                    from stores.store_scope import chunk_matches_store, get_current_store
    ...
    try:
        try:
            from catalog.seller_catalog import seller_as_catalog_products
        except ImportError:
            from catalog.seller_catalog import seller_as_catalog_products
        ...
        for sp in seller_as_catalog_products(active_only=True):
            try:
                try:
                    from stores.store_scope import product_matches_store, get_current_store
                except ImportError:
                    from stores.store_scope import product_matches_store, get_current_store
