...
so images always match the product type in our catalog. Not official Adidas photos.
...
try:
    ...
    from product_image_pools import pool_key_for_product, urls_for_product
except ImportError:
    from .adidas_catalog import all_products
    from .product_image_pools import pool_key_for_product, urls_for_product

OUT_DIR = Path(__file__).resolve().parent / "static" / "products"
MANIFEST = Path(__file__).resolve().parent / "product_images.json"
MIN_BYTES = 5_000
