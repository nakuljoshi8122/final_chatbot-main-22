...
try:
    from .conversation_context import infer_use_case
except ImportError:
    from conversation_context import infer_use_case

try:
    from .adidas_catalog import all_products as _all_products
except ImportError:
    from adidas_catalog import all_products as _all_products
...
def match_products_constrained(
    ...
    if strict:
        try:
            from .tile_validator import find_catalog_product, product_to_tile
        except ImportError:
            from tile_validator import find_catalog_product, product_to_tile
        tiles: list[dict] = []
        for card in strict[:max_results]:
            ...
            if not raw:
                continue
            try:
                from .tile_validator import product_to_tile
            except ImportError:
                from tile_validator import product_to_tile
