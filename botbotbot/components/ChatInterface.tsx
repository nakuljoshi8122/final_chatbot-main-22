...
import ConversationalVoiceScreen from './ConversationalVoiceScreen';
...
export default function ChatInterface({ initialSessionId }: ChatInterfaceProps) {
  ...
  const [showPseudoDuplex, setShowPseudoDuplex] = useState(false);
  ...
  if (showPseudoDuplex) {
    return <PseudoDuplexVoiceScreen onClose={() => setShowPseudoDuplex(false)} />;
  }
  if (showVoiceConversation) {
    return <ConversationalVoiceScreen onClose={() => setShowVoiceConversation(false)} />;
  }
